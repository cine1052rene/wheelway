...
방법론 (반드시 아래 규칙을 유지할 것 — 데이터 조작 없이 재현 가능해야 함):
  ...
  - doorWidthCm(엘리베이터 문 폭)은 이 공공데이터에 필드 자체가 없어 실측 불가.
    지어낸 값(과거엔 법정 최소 90cm를 하드코딩)으로 라우팅 통과/차단을 결정하면
    실제 이용자를 오도할 수 있어 위험하므로, doorWidthCm=null(실측 전)로 두고
    doorWidthStatus='실측 전'으로 표시함. 라우팅은 실측 전 문 폭으로 게이팅하지 않으며,
    실제 폭이 확보되면 doorWidthCm에 숫자를 넣고 doorWidthStatus=None으로 두면 반영됨.
...
for name in all_names:
    ...
    stations[name] = {
        ...
        # 문 폭은 이 공공데이터에 필드가 없어 실측 불가.
        # 지어낸 값(과거: 90 하드코딩)으로 라우팅을 게이팅하면 위험하므로 null로 두고,
        # 상태를 '실측 전'으로 표기한다. 실제 폭이 확보되면 doorWidthCm에 숫자를 넣고 status는 null.
        'doorWidthCm': None,
        'doorWidthStatus': '실측 전' if has_elevator else None,
        'note': note,
    }
...
stations_out.append("// doorWidthCm(엘리베이터 문 폭)은 이 공공데이터에 없어 실측 전 상태(null)입니다.")
stations_out.append("// 라우팅은 실측되지 않은 문 폭으로 통과/차단을 결정하지 않으며, doorWidthStatus로만 안내합니다.")
stations_out.append("export const stations = [")
for name in sorted(stations.keys()):
    s = stations[name]
    door_cm_js = 'null' if s['doorWidthCm'] is None else str(s['doorWidthCm'])
    door_status_js = 'null' if s['doorWidthStatus'] is None else "'%s'" % esc(s['doorWidthStatus'])
    stations_out.append(
        "  { id: '%s', name: '%s', lines: [%s], elevator: %s, escalator: %s, capacityKg: %d, doorWidthCm: %s, doorWidthStatus: %s, note: '%s' },"
        % (
            esc(name), esc(name),
            ', '.join(f"'{l}'" for l in s['lines']),
            'true' if s['elevator'] else 'false',
            'true' if s['escalator'] else 'false',
            s['capacityKg'], door_cm_js, door_status_js,
            esc(s['note']),
        )
    )
stations_out.append("];")
